...
export default function BusInchargeAuth() {
    ...
    useEffect(() => {
        ...
        if (token && role === 'bus-incharge') {
            navigate('/bus-incharge/dashboard')
        }
    }, [])
    ...
    const submit = async e => {
        ...
        try {
            ...
            if (res.data.role !== 'bus-incharge') return alert('Not a bus incharge account')
            localStorage.setItem('token', res.data.token)
            localStorage.setItem('role', res.data.role)
            localStorage.setItem('username', res.data.username)
            localStorage.setItem('email', res.data.email || res.data.username)
            // Navigate to bus incharge dashboard
            navigate('/bus-incharge/dashboard')
        } catch (err) {
        ...
        <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 text-white px-4">
            <div className="relative w-full max-w-md p-8 rounded-2xl shadow-2xl border border-slate-700 bg-slate-900/60 backdrop-blur-xl">
                <div className="text-center mb-6">
                    ...
                    <h2 className="text-3xl font-bold tracking-tight">Bus Incharge Login</h2>
                    <p className="text-slate-400 text-sm mt-1">
                        Bus Incharges can log in to view and manage bus operations.
                    </p>
                </div>
